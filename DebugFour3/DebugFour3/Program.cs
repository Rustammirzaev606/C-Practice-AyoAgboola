...
class DebugFour3
{
    static void Main()
    {
        ...
        if (credits <= LOWCREDITS)
        {
            if (year == SENIORYEAR)
            {
                tuition = (HOURFEE * credits) - ((HOURFEE * credits) * DISCOUNT);
            }
            else
            {
        ...
        else if (credits > HIGHCREDITS)
        {
            if (year == SENIORYEAR)
            {
                tuition = (FLAT + ((credits - HIGHCREDITS) * RATE)) - ((FLAT + ((credits - HIGHCREDITS) * RATE)) * DISCOUNT);
            }
            else
            {
